...
namespace TheBugTracker.Services
{
    public class BTNotificationService : IBTNotificationService
    {
        ...
        public async Task<List<Notification>> GetReceivedNotificationsAsync(string userId)
        {
            try
            {
                List<Notification> notifications = await _context.Notifications  //.Where(n => n.RecipientId == userId).ToListAsync();
        ...
        public async Task SendEmailNotificationsByRoleAsync(Notification notification, int companyId, string role)
        {
            try
            {
                List<BTUser> btUsersInRole = await _rolesService.GetUsersInRoleAsync(role, companyId);

                try
                {
                    foreach(BTUser user in btUsersInRole)
                    {
